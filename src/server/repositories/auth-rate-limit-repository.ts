  ...
  async consume(input) {
    return db.transaction(async (tx) => {
      ...
      if (!row || input.nowMs - row.lastRequest >= windowMs) {
        await tx.insert(rateLimits).values({ id: randomUUID(), key: input.key, count: 1, lastRequest: input.nowMs }).onDuplicateKeyUpdate({ set: { count: 1, lastRequest: input.nowMs } });
